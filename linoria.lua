...
for _, path in next, libraryy.folders do 
    makefolder(libraryy.directory .. path)
end 

if not isfile(libraryy.directory .. "/fonts/proggyclean.ttf") then
    writefile(libraryy.directory .. "/fonts/proggyclean.ttf", game:HttpGet("https://github.com/f1nobe7650/Nebula/raw/refs/heads/main/fs-tahoma-8px.ttf"))
end

local ProggyCleanFace = {
    name = "ProggyClean",
    faces = {
        {
            name = "Regular",
            weight = 400,
            style = "normal",
            assetId = getcustomasset(libraryy.directory .. "/fonts/proggyclean.ttf")
        }
    }
}

writefile( libraryy.directory .. "/fonts/ProggyClean.font.json", game:GetService("HttpService"):JSONEncode(ProggyCleanFace))

libraryy.font = Font.new( getcustomasset(libraryy.directory .. "/fonts/ProggyClean.font.json"), Enum.FontWeight.Regular)
...
local TabButton = Library:Create('Frame', {
    ...
    BorderColor3 = Library.OutlineColor;
    BorderSizePixel = 1;
    Position = UDim2.new(0, 0, 0, 2); -- push down from top slightly
Size = UDim2.new(0, TabButtonWidth + 8 + 4, 1, -10);
        ZIndex = 1;
    Parent = TabArea;
});
    ...
    local strokeee = Library:Create('UIStroke', {
    Color = Library.OutlineColor;
    Thickness = 1;
    ApplyStrokeMode = Enum.ApplyStrokeMode.Border;
    Parent = TabButton;
});
    Library:AddToRegistry(strokeee, {
        Color = 'OutlineColor';
    });

    -- Accent line at the top of the selected tab
    ...
    function Tab:ShowTab()
        ...
        -- Show accent line and update colors for selected tab
    ...
    function Tab:HideTab()
        -- Hide accent line and restore default colors
